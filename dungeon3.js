...
const esaimg = new Image();
esaimg.src = "dneko4.png";
...
function repaint(){
    ...
    for(let x = 0; x < W; x++){
        for(let y = 0; y < H; y++){
            if(maze[y][x] == 1){
                ctx.drawImage(wallimg, x*size, y*size, size, size);
            }
        }
    }

    ctx.drawImage(esaimg,goalx*size,goaly*size,30,30);
...
function Player(x,y){
    ...
    this.paint = function(gc,x,y,w,h){
        let img = document.getElementById("dneko" + this.dir);
        //gc.drawImage(img,x,y,w,h); //教科書
        gc.drawImage(img, this.x*size, this.y*size, size, size); //主人公描画
    };
}
